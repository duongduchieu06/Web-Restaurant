  ...
  const AdminManageFood = () => {
    ...
    const handleDetailMeal = () => {
      ...
      const dataTable = meals?.data?.length
        ? meals.data
              .filter((meal) =>
                  meal.name.toLowerCase().includes(searchText.toLowerCase())
              )
              .map((meal) => ({ ...meal, key: meal._id }))
        : [];
        ...
        >
          <WrapperEdit>
            <BoxWrapper>
              ...
              <BoxInput>
                <Label>Loại:</Label>
                <SelecteStyled value={stateMealDetail.type} onChange={(e) => handleOnChangeDetail("type", e.target.value)}>
